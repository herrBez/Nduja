"""Module of etherscan transaction retriever class"""
...
class EtherscanTransactionRetriever(AbsTransactionRetriever):
    ...
    def build_query(self, address: str):
        ...
        if self.tokens is not None and self.tokens:
    ...
    def get_input_output_addresses(self, address: str,
        ...
        response = safe_requests_get(query=query, token=None)
        ...
        if response is None:
            logging.warning("EtherscanTransactionRetriever %s failed", query)
        else:
            ...
            for tr_ in txs:
                from_address = tr_["from"].strip()
                to_address = tr_["to"].strip()
